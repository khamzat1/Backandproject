...
router.put('/:id',async (req,res) => {
    ...
    let customer1 = await Customer.findByIdAndUpdate(req.params.id,{isVip:req.body.isVip},
    {new:true});

    let customer2 = await Customer.findByIdAndUpdate(req.params.id,{phone:req.body.phone},
    {new:true});

    if(!customer)
        return res.status(404).send('Berilgan IDga teng bulgan toifa topilmadi');
     
    res.send(customer,customer1,customer2);
